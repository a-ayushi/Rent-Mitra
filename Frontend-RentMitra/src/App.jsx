...
const AppContent = () => {
  ...
  return (
    <div className={`flex flex-col ${isAuthRoute ? "h-screen" : "min-h-screen"}`}>
      {!isAuthRoute && <Navbar />}
      ...
      <main className="flex-grow">
        <Routes>
          ...
          <Route path="/search" element={<Search />} />
          <Route path="/categories" element={<Categories />} />
          <Route path="/categories/:category" element={<Search />} />
      ...
      {/* ✅ Floating ChatBot (Guest + User) */}
      {!isAuthRoute && <ChatBot />}

      {/* -------- FOOTER -------- */}
      {isAuthRoute ? (
        <footer className="bg-gray-900 text-white mt-auto">
          <div className="max-w-7xl mx-auto py-4 px-4 flex justify-between text-sm text-gray-400">
            <span>© 2025 Rentra Mitra</span>
            <div>
